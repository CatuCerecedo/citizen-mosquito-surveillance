...
cluster <- tes_hot_spots %>%
  ...
  mutate(
    classification = case_when(
      ...
      gi > 0 & p_folded_sim <= 0.05 ~ paste0(fldr_ma, ">", fldr),
...
ggpubr::ggarrange(
  ggplot(cluster_wth %>% filter(classification!= "Insignificant")) +
    geom_boxplot(aes(x = classification, y = min_temperature)) +
    theme_classic(),
  ggplot(cluster_wth %>% filter(classification!= "Insignificant")) +
    geom_boxplot(aes(x = classification, y = max_temperature)) +
    theme_classic(),
  ggplot(cluster_wth %>% filter(classification!= "Insignificant")) +
...
ggpubr::ggarrange(
  ggplot(cluster_wth %>% filter(gi > 0)) +
    geom_boxplot(aes(x = classification, y = min_temperature)) +
    theme_classic(),
  ggplot(cluster_wth %>% filter(gi > 0)) +
    geom_boxplot(aes(x = classification, y = max_temperature)) +
