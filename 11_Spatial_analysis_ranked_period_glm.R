...
period = "winter"
months = c("01", "02", "03", "04", "05", "06", "10", "11", "12")
...
for(i in 1:length(months)){
  ...
  if (i == 1){
    ...
    pred_ma$count <- rowMeans(pred_ma[,6:ncol(pred_ma)], na.rm = TRUE)
    
    pred_ma <- pred_ma[c("municipality","id", "prov_name", "lon", "lat", "count")]
  } else {
...
ggsave(paste0(loc.fig, "Spatial_clusters/period_ranked_", y, "_", fldr, "_", fldr_ma, "_", period, ".png"),
...
ggsave(paste0(loc.fig, "Spatial_clusters/period_diff_ranked_", y, "_", fldr, "_", fldr_ma, "_", period, ".png"),
       width = 25,  height = 25, units = "cm", bg = "white")
...
ggsave(paste0(loc.fig, "Spatial_clusters/period_clusters_", y, "_", fldr, "_", fldr_ma, "_", period, ".png"),
       width = 25,  height = 25, units = "cm", bg = "white")
...
saveRDS(cluster_wth, paste0(loc.output, "cluster_", y, "_", fldr, "_", fldr_ma, "_", period, ".rds"))
...
ggsave(paste0(loc.fig, "Spatial_clusters/period_cluster_boxplot_", y, "_", fldr, "_", fldr_ma, "_", period, ".png"),
...
ggsave(paste0(loc.fig, "Spatial_clusters/period_cluster_density_",  y, "_", fldr, "_", fldr_ma, "_", period, ".png"),
       width = 25,  height = 25, units = "cm")

# GLM: logistic regression -----------------------------------------------------
cluster_wth <- readRDS(paste0(loc.output, "cluster_", y, "_", fldr, "_", fldr_ma, "_", period, ".rds")) %>%
  mutate(classification = factor(classification, levels = c(paste0(fldr, ">", fldr_ma),  paste0(fldr, "<", fldr_ma)))) %>%
  filter(classification != "Insignificant")
...
ggsave(paste0(loc.fig, "Spatial_clusters/glm/period_boots_",  y, "_", fldr, "_", fldr_ma, "_", period, ".png"),
       width = 35,  height = 22, units = "cm")

# GLM: linear regression -------------------------------------------------------
cluster_wth <- readRDS(paste0(loc.output, "cluster_", y, "_", fldr, "_", fldr_ma, "_", period, ".rds")) 

linear_regression <- glm(gi ~ min_temperature + mean_temperature + mean_relative_humidity + precipitation, 
                      data = cluster_wth)
...
car_tab <- cluster_wth %>%
  dplyr::select(all_of(var), gi) %>%
  rename(value = all_of(var))
a <- ggplot(car_tab, aes(x = value, y = gi)) +
  geom_point(alpha = .3, color = "gray50") +
  geom_smooth(method = "lm", se = TRUE, color = "orangered2") +
  labs(x = var, y = "Getis Org Gi Statistic",
       caption = paste0("Estimate (SE): ", round(linear_regression$coefficients[var], 3), 
                        " (", round(summary(linear_regression)$coefficients[7], 3), ")\n",
                        "P-value: ", round(summary(linear_regression)$coefficients[17], 5))
  ) +
  theme_classic()
l_0 = levels(cluster_wth$classification)[1]
l_1 = levels(cluster_wth$classification)[2]
b <- ggplot(cluster_wth %>% filter(classification == l_0), 
            aes(x = min_temperature, y = gi)) +
  geom_point(alpha = .3, color = "gray50") +
  geom_smooth(method = "lm", se = TRUE, color = "orangered2") +
  labs(x = var, y = paste("Getis Org Gi Statistic\n[Classification = ", l_0, "]")) +
  theme_classic()
c <- ggplot(cluster_wth %>% filter(classification == l_1), 
            aes(x = min_temperature, y = gi)) +
  geom_point(alpha = .3, color = "gray50") +
  geom_smooth(method = "lm", se = TRUE, color = "orangered2") +
  labs(x = var, y = paste("Getis Org Gi Statistic\n[Classification = ", l_1, "]")) +
...
a <- ggplot(car_tab, aes(x = value, y = gi)) +
  ...
  theme_classic()
b <- ggplot(cluster_wth %>% filter(classification == l_0), 
            aes(x = mean_relative_humidity, y = gi)) +
  geom_point(alpha = .3, color = "gray50") +
  geom_smooth(method = "lm", se = TRUE, color = "orangered2") +
  labs(x = var, y = paste("Getis Org Gi Statistic\n[Classification = ", l_0, "]")) +
  theme_classic()
...
ggpubr::ggarrange(boo[[1]], boo[[2]], boo[[3]], boo[[4]])

ggsave(paste0(loc.fig, "Spatial_clusters/glm/period_boots_",  y, "_", fldr, "_", fldr_ma, "_", period, ".png"),
       width = 35,  height = 35, units = "cm")

# Plotting climatic variables between periods ----------------------------------
plot_wth <- list()
period = "summer"
months = c("07", "08", "09")
period = "winter"
...
for (i in 1:length(months)){
  wth <- readRDS(paste0(loc.output, "monthly_weather_data/prep_", 
                        months[1], "-", y, ".rds"))[, c(2,9:12)]
  wth_full[[i]] <- wth
}
wth_full <- bind_rows(wth_full) %>%
  group_by(id) %>%
  summarise(
    min_temperature = mean(min_temperature, na.rm = TRUE), 
    mean_temperature = mean(mean_temperature, na.rm = TRUE), 
    precipitation = mean(precipitation, na.rm = TRUE), 
    mean_relative_humidity = mean(mean_relative_humidity, na.rm = TRUE), .groups = "drop")

wth_full <- merge(spain, wth_full, by = "id",)

mint <- ggplot() + 
  geom_sf(data = wth_full, aes(fill = min_temperature), color = "transparent", size = 0.2) +
  scale_fill_distiller("Minimum Temperature", palette = "Spectral") +
  theme_classic() +
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
meant <- ggplot() + 
  geom_sf(data = wth_full, aes(fill = mean_temperature), color = "transparent", size = 0.2) +
  scale_fill_distiller("Mean Temperature", palette = "Spectral") +
  theme_classic()  +
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
mrh <- ggplot() + 
  geom_sf(data = wth_full, aes(fill = mean_relative_humidity), color = "transparent", size = 0.2) +
  scale_fill_distiller("Mean Relative\nHumidity", palette = "Spectral") +
  theme_classic() +
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
ppt <- ggplot() + 
  geom_sf(data = wth_full, aes(fill = precipitation), color = "transparent", size = 0.2) +
  scale_fill_distiller("Precipitation", palette = "Spectral") +
  theme_classic() +
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))

plot_wth[[2]] <- (mint + meant) / (mrh + ppt) +
  plot_annotation(subtitle = "Non Mosquito Period", tag_levels = c("A"), tag_suffix = ")") 


(plot_wth[[1]]) / (plot_wth[[2]])

plot_wth[[2]] 

ggsave(paste0(loc.fig, "Spatial_clusters/glm/period_wth_",  y, "_2.png"),
